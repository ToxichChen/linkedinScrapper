...
class DBManager {
  ...
  async getNotConnectedUsersArray() {
    let sql = ('SELECT `linkedinUrl` FROM `clients` WHERE `connectSent` = 0 LIMIT 3');
    return await new Promise((resolve, reject) => {
      this.connection.query(sql, async function (err, result) {
        resolve(result);
      });
  ...
  async setConnected() {
    return await new Promise((resolve, reject) => {
      for (const value of urlsArray) {
        this.connection.query('UPDATE `clients` SET `connectSent` = 1 WHERE `linkedinUrl` = ' + value, async function (err, result) {
